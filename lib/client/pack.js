...
var cleanCSS, deleteOldFiles, existslib, formatKb, fs, magicPath, mkdir, pathlib, system, view;
...
module.exports = function(ss, client, options) {
  ...
  packAssetSet = function(assetType, paths, dir, postProcess) {
    ...
    writeFile = function(fileContents) {
      ...
      return ss.log('✓'.green, 'Packed ' + filePaths.length + ' files into ' + fileName.substr(ss.root.length));
    };
  ...
  ss.log(("Pre-packing and minifying the '" + client.name + "' client...").yellow);
  ...
  packAssetSet('css', client.paths.css, options.dirs.css, function(files) {
    ...
    ss.log(("  Minified CSS from " + (formatKb(original.length)) + " to " + (formatKb(minified.length))).grey);
    return minified;
  });
  ...
  return view(ss, client, options, function(html) {
    ...
    return ss.log('✓'.green, 'Created and cached HTML file ' + fileName.substr(ss.root.length));
  });
};
...
deleteOldFiles = function(clientDir) {
  ...
  return filesDeleted.length > 1 && ss.log('✓'.green, "" + filesDeleted.length + " previous packaged files deleted");
};
